...
void main() {
  test('v0.16.5 uses the permanent collision-free Android identity', () {
    final pubspec = File('pubspec.yaml').readAsStringSync();
    final gradle = File('android/app/build.gradle.kts').readAsStringSync();

    expect(pubspec, contains('version: 0.16.5+4045'));
    final version = RegExp(r'version:\s*[^+]+\+(\d+)').firstMatch(pubspec);
    expect(version, isNotNull);
    expect(
      int.parse(version!.group(1)!),
      greaterThan(4043),
      reason: 'Android updates must exceed the published v0.16.3 build code.',
    );
